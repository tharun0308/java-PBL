...
export async function POST(
  ...
  try {
    ...
    const { id } = params;
    const response = await apiClient<any>(`/api/v1/admin/staff-requests/${id}/reject`, {
      method: 'POST',
      token,
    });
    ...
    return NextResponse.json({
      success: true,
      message: 'Staff Admin request rejected',
      data: response,
    });
  } catch (err: any) {
    const status = err instanceof ApiException ? err.status : 500;
    return NextResponse.json(
      { error: err.message || 'Failed to reject staff admin request' },
      { status }
    );
  }
}
